	...
	Case of 
		: ($type=Is text:K8:3)
			...
			Case of 
				...
				: (cs:C1710.is.me.json_collection($something))  // is
		...
		: ($type=Is pointer:K8:14)
			...
			Case of 
				: ($type=Array 2D:K8:24)
					...
					For ($i; 1; Size of array:C274($something->))
						
						$_c:=[]
						ARRAY TO COLLECTION:C1563($_c; $something->{$i})
				...
				: (cs:C1710.is.me.array($something))  // is
					
					
					ARRAY TO COLLECTION:C1563($_; $something->)
...
Function object($something : Variant) : Object
	
	var $buffer : Text
	
	Case of 
			
		: (cs:C1710.is.me.text($something))  // is
			
			If (cs:C1710.is.me.json_object($something))  // is
				return JSON Parse:C1218($something)
			End if 
			
			
			
		: (cs:C1710.is.me.file($something))  // is
			
			If ($something.exists)
				$buffer:=$something.getText()
